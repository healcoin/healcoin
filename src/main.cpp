...
#if defined(NDEBUG)
# error "Healcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x5de1108cd698eeb8fd017f52e35ac21e3bdc3f0dc653e7b1e3863c073b0be978");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Healcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Healcoin Signed Message:\n";
...
{
    // Healcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    }else if(nHeight <= 5734){
    	nSubsidy = 500000 * COIN;
   }else if(nHeight <= 22939067){
        nSubsidy = 500 * COIN;
   }else if(nHeight <= 22939068){
        nSubsidy = 166 * COIN;
   }
...
static const int64 nTargetTimespan = 14 * 24 * 60 * 60; // Healcoin: 14 days
static const int64 nTargetSpacing = 2 * 60; // Healcoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Healcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Healcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x5de1108cd698eeb8fd017f52e35ac21e3bdc3f0dc653e7b1e3863c073b0be978");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is Healcoin. Date:-24/02/2018";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("472484710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1519551501;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2907609;
        ...
        assert(block.hashMerkleRoot == uint256("0x3d5a6e17d0ffa492b5757dfcc780e920eee879adaa828b8aa0bce3f0e05bce12"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Healcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("HealcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("HealcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static HealcoinMiner(CWallet *pwallet)
{
    printf("HealcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("healcoin-miner");
    ...
    try { loop {
        ...
        printf("Running HealcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("HealcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&HealcoinMiner, pwallet));
}
